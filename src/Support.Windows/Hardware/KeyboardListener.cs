...
namespace Platform.Support.Windows.Hardware
{
    ...
    //
    // Summary:
    //     Represents the method that will handle the System.Windows.Forms.Control.KeyUp
    //     or System.Windows.Forms.Control.KeyDown event of a System.Windows.Forms.Control.
    //
    // Parameters:
    //   sender:
    ...
    public class KeyboardListener : HardwareListener
    {
        public KeyboardListener() : base(WH_KEYBOARD_LL)
        {
        }
        ...
        internal override IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                ...
                IsHeld = wParam == (IntPtr)WM_KEYDOWN && wParam == LastEvent;

                var hookStruct = (KBDLLHOOKSTRUCT)Marshal.PtrToStructure(lParam, typeof(KBDLLHOOKSTRUCT));
                //OnDebug(new EventArgs<string>($"held: {IsHeld}, key:{ key}, nCode: { nCode}, wParam: { wParam}, lParam: { lParam}, " + hookStruct.ToString()));

                if (wParam == (IntPtr)WM_KEYDOWN)
                {
                    KeyDown?.Invoke(null, new KeyEventArgs(key) { SuppressKeyPress = IsHeld });
                }
                else if (wParam == (IntPtr)WM_KEYUP)
                {
                    KeyUp?.Invoke(null, new KeyEventArgs(key) { SuppressKeyPress = IsHeld });
                    KeyPress?.Invoke(null, new KeyEventArgs(key) { SuppressKeyPress = IsHeld });
                }
            }
