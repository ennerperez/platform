...
namespace Platform.Support.Windows
{
    public static class OS
    {
        ...
        {
            using (RegistryKey registryKey = Microsoft.Win32.Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\Session Manager"))
                if (registryKey != null && registryKey.GetValue("PendingFileRenameOperations") != null)
                    return true;
            bool result;
            using (RegistryKey registryKey2 = Microsoft.Win32.Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\WindowsUpdate\\Auto Update\\RebootRequired"))
